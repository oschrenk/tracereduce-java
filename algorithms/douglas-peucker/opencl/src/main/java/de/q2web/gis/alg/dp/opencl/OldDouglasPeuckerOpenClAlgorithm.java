...
public class OldDouglasPeuckerOpenClAlgorithm implements Algorithm {
	...
	private void run() {
		...
		// run recursive
		run(leftOffset, rightOffset, length, fromX, fromY, toX, toY);
	...
	private void run(final int leftOffset, final int rightOffset, int length,
		...
		clSetKernelArg(distanceKernel, 4, Sizeof.cl_uint,
		...
		clSetKernelArg(distanceKernel, 8, Sizeof.cl_float,
