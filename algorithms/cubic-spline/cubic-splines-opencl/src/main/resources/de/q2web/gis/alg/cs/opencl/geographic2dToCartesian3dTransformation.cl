...
__kernel void cubicSpline(
	...
	// 		III	b_i + 2c_ix_{i+1} + 3d_ix^2_{i+1} - b_{i+1} - 2c_{i+}}x_{i+1} + 3d_{i+1}}x^2_{i+1} = 0
	...
	//$4	At the left end of the curve we are missing the C_1 and C_2 equations
	...
	// =>
		...
		for (i = 1; i < length -1; i++) {
			...
			fPrimeLeft = 	i != 1
							? 		2 / ((xb - xa) / (yb - ya) + (xa - x0) / (ya - y0))
							: 		3f / 2f * (yb - ya) / (xb - xa) - fPrimeRight	/ 2;

			if (i == length) {
				fPrimeRight = 		3f / 2f * ((yb - ya) / (xb - xa)) - fPrimeLeft / 2;
			}

			fDoublePrimeLeft = 	  - (2 * (fPrimeRight + 2 * fPrimeLeft)) / (xb - xa)
								  + (6 * (yb - ya) / (xb - xa)) / (xb - xa);
			fDoublePrimeRight =      2 * (2 * fPrimeRight + fPrimeLeft) / (xb - xa)
								   - 6 * (yb - ya) / (xb - xa) / (xb - xa);

			d[i] = 1f / 6f * (fDoublePrimeRight - fDoublePrimeLeft) / (xb - xa);
			c[i] = 1f / 2f * (xb * fDoublePrimeLeft - xa * fDoublePrimeRight) / (xb - xa);
			b[i] = (
					(yb - ya)
					- c[i] * (xb * xb - xa * xa)
					- d[i] * (xb * xb * xb - xa * xa * xa)
				) / (xb - xa);
